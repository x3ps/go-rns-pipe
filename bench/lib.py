"""
lib.py — Reusable benchmark logic for go-rns-pipe.

All test runners, helpers, plotting functions, and utilities live here so that
bench.py can be a thin CLI wrapper and test_bench.py can import from a stable,
non-colliding module path (bench.lib).
"""
...
def wait_for_reflector_hash(timeout: int = 60) -> str:
    deadline = time.time() + timeout
    last_error = None
    while time.time() < deadline:
        ...
        time.sleep(2)
    raise RuntimeError(f"Reflector hash not available after {timeout}s: {last_error}")
...
def run_test_reconnect(reticulum, dest_hash_hex: str, docker: bool) -> dict:
    console.print("[bold cyan]Test 4: Reconnect after rns-tcp-iface restart[/bold cyan]")
    result = {
        "test": "reconnect",
        "docker": docker,
        "downtime_s": None,
    ...
    # Kill rns-tcp-iface on node-a; rnsd will respawn it via respawn_delay=2
    kill_result = subprocess.run(
        ["docker", "exec", "rns-node-a", "pkill", "-f", "rns-tcp-iface"],
        capture_output=True,
    )
    if kill_result.returncode != 0:
        console.print("  [red]pkill returned non-zero — rns-tcp-iface was not running[/red]")
        return result

    console.print("  Killed rns-tcp-iface, waiting for it to go offline...")

    # Phase 1: wait until process is gone (offline)
    t_offline = None
    deadline = time.time() + 15
    while time.time() < deadline:
        rc = subprocess.run(
            ["docker", "exec", "rns-node-a", "pgrep", "-f", "rns-tcp-iface"],
            capture_output=True,
        ).returncode
        if rc != 0:
            t_offline = time.time()
    ...
    if t_offline is None:
        console.print("  [red]Process did not go offline within 15s[/red]")
        return result

    console.print("  Process offline, waiting for respawn...")

    # Phase 2: wait until process is back (online)
    t_online = None
    ...
    if t_online is None:
        console.print("  [red]Did not respawn within 30s[/red]")
        return result

    downtime_s = t_online - t_offline
    result["downtime_s"] = downtime_s
    result["reconnected"] = True
    console.print(f"  Reconnected after {downtime_s:.1f}s downtime")
...
def run_test_hdlc_integrity(pipe_bridge_bin: str) -> dict:
    ...
    # Each decoded frame produces one output line (empty frame → empty line)
    lines = stdout_data.decode().splitlines()
    ...
    # Empty frame: 0 bytes payload → 1 empty packet (empty line)
...
def plot_reconnect_timeline(result: dict, outdir: str):
    ...
    if result.get("downtime_s") is not None:
        ...
        ax.barh(["rns-tcp-iface"], [downtime], color="salmon", label="downtime")
