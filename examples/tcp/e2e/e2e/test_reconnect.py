"""E2E: respawn + path recovery after rns-tcp-iface kill."""
...
def test_respawn_and_recovery(rns_client, reflector_hashes):
    """Path recovers within 60s of rns-tcp-iface kill on node-a."""
    ...
    # Kill rns-tcp-iface on node-a
    assert _kill_iface(), "kill-iface failed — rns-tcp-iface was not running"

    # Wait for process to go offline
    wait_until(lambda: not _check_iface(), timeout=15, desc="rns-tcp-iface offline")

    # Wait for process to respawn (rnsd respawn_delay=2)
    wait_until(_check_iface, timeout=30, desc="rns-tcp-iface respawned")
